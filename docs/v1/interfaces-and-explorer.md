...
The normative contract is [`protocol/api/openapi-v1.yaml`](../../protocol/api/openapi-v1.yaml). Bootstrap `GET /api/status` returns exact chain/genesis/protocol/API/release identity, separate unsafe, justified, and finalized coordinates, freshness, and evidence-registry root. Core v1 reads cover blocks, transactions, notes, address balance/history, nodes, workers, objects, models, jobs/chunks, receipts, disputes, and mechanism evidence. `POST /api/v1/transactions` accepts exactly the noosd envelope `{"tx":"<lowercase hex>","witnesses":"<lowercase hex>"}`. Forwarding is enabled only when the indexer has `NOOS_NODE_RPC`, `NOOS_NODE_TOKEN`, `NOOS_CHAIN_ID`, and `NOOS_GENESIS_HASH`; it verifies the authenticated node `/status` identity immediately before sending the exact request bytes to `/submit_tx`. Missing configuration, identity mismatch, transport failure, refusal, or malformed node output fails closed. A successful response contains only the node-accepted txid and never creates a local `MEMPOOL` row: node ingestion remains the sole source of indexed transaction truth. Requests are capped at 1 MiB, operator responses at 64 KiB, and operator connect/read/write operations at five seconds. Hash IDs are lowercase 64-hex, quantities are base-10 strings, addresses are canonical lowercase `noos` Bech32m, pages have stable sort and opaque query-bound cursors. Disabled mechanism routes return HTTP 409 `feature_disabled` with mechanism and evidence reference, never empty success.
