//! Writes `protocol/vectors/da/*.json` from the shared deterministic
//! generator (`noos_da::vector_gen`). The crate tests verify the on-disk
...
fn main() {
    ...
    for (name, file) in noos_da::vector_gen::files() {
        let path = dir.join(name);
        let json = noos_da::vector_gen::render_json(&file);
