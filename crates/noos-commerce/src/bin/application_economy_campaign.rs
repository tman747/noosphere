use noos_commerce::campaign::run_application_economy_campaign;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("application economy campaign failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let report = run_application_economy_campaign(&source_revision, seed)
        .map_err(|error| format!("{error:?}"))?;
    write_immutable_json(&output, &report)?;
    println!(
        "application economy campaign PASS: {} scenarios -> {}",
        report.steps.len(),
        output.display()
    );
    Ok(())
