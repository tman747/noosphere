...
fn main() {
    ...
    // -- positives: primitive layouts ----------------------------------------
    {
        ...
        cases.push(positive(
            "prim_array32",
            w.into_bytes(),
            "fixed 32-byte array, no length prefix",
        ));
    }
    ...
    // -- positives: collections ----------------------------------------------
    {
        ...
        cases.push(positive(
            "bytes_at_max",
            w.into_bytes(),
            "collection exactly at type max 16",
        ));
        let mut w = Writer::new();
        ...
        cases.push(positive(
            "atom_256_le_minimal",
            w.into_bytes(),
            "atom 256: trailing zero ok, leading zero banned",
        ));
        let mut w = Writer::new();
        ...
        cases.push(positive(
            "optional_field",
            w.into_bytes(),
            "optional tag 0x8001 with 3-byte payload",
        ));
    }
    ...
    // unknown version
    {
        ...
        b[1] = 0xFF;
        cases.push(negative(
            "demo_version_high_byte",
            b,
            CodecError::UnknownVersion,
            "version 0xFF01 unknown",
        ));
    }
    ...
    // unknown mandatory field
    {
        ...
        b[2] = 0x02;
        cases.push(negative(
            "demo_tag_order_swapped",
            b,
            CodecError::UnknownMandatoryField,
            "tag out of declared order",
        ));
        let mut b = base.clone();
        b[3] = 0x80;
        cases.push(negative(
            "demo_optional_tag_in_mandatory_slot",
            b,
            CodecError::UnknownMandatoryField,
            "optional-range tag where mandatory expected",
        ));
    }
    ...
    // length bound violations (standalone byte-string context, max=16)
    {
        ...
        cases.push(negative(
            ...
            "17 > max 16 (ctx: get_bytes max=16)",
        ));
        let mut w = Writer::new();
        w.put_u32(0xFFFF_FFFF);
        cases.push(negative(
            "bytes_huge_prefix_no_alloc",
            w.into_bytes(),
            CodecError::LengthExceedsBound,
            "0xFFFFFFFF length on empty tail must fail before allocation (ctx: get_bytes max=16)",
        ));
        let mut w = Writer::new();
        w.put_u32(8);
        w.put_raw(&[0xBB; 4]);
        cases.push(negative(
            "bytes_len_exceeds_remaining",
            w.into_bytes(),
            CodecError::LengthExceedsBound,
            "declared 8, only 4 remain (ctx: get_bytes max=16)",
        ));
    }
    ...
    // nonminimal atom (standalone atom context, max=16)
    {
        ...
        cases.push(negative(
            "atom_leading_zero",
            w.into_bytes(),
            CodecError::NonMinimalAtom,
            "leading zero byte (ctx: get_atom max=16)",
        ));
        let mut w = Writer::new();
        w.put_u32(1);
        w.put_raw(&[0x00]);
        cases.push(negative(
            "atom_single_zero_byte",
            w.into_bytes(),
            CodecError::NonMinimalAtom,
            "zero must be empty, not 00 (ctx: get_atom max=16)",
        ));
    }
    ...
    // discriminant (standalone context, variant_count=3)
    {
        ...
        w.put_u16(3);
        cases.push(negative(
            "discriminant_eq_count",
            w.into_bytes(),
            CodecError::UnknownDiscriminant,
            "3 invalid for 3 variants (ctx: get_discriminant 3)",
        ));
        let mut w = Writer::new();
        w.put_u16(0xFFFF);
        cases.push(negative(
            ...
            w.into_bytes(),
            CodecError::UnknownDiscriminant,
            "0xFFFF invalid (ctx: get_discriminant 3)",
        ));
    }
    ...
    // list violations (standalone context, list of u64, max=8)
    {
        ...
        cases.push(negative(
            "list_count_exceeds_max",
            w.into_bytes(),
            CodecError::LengthExceedsBound,
            "9 > max 8 (ctx: get_list<u64> max=8)",
        ));
        let mut w = Writer::new();
        w.put_u32(3);
        w.put_u64(1);
        ...
        cases.push(negative(
            ...
            w.into_bytes(),
            CodecError::Truncated,
            "third u64 short (ctx: get_list<u64> max=8)",
        ));
        let mut w = Writer::new();
        w.put_u32(0xFFFF_FFFF);
        w.put_raw(&[0u8; 8]);
        cases.push(negative(
            "list_huge_count_no_alloc",
            w.into_bytes(),
            CodecError::LengthExceedsBound,
            "forged count fails byte-floor check (ctx: get_list<u64> max=2^32-1)",
        ));
    }
    ...
    let root =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../protocol/vectors/codec");
    ...
    println!(
        "wrote {} cases ({pos} positive, {neg} negative) to {}",
        cases.len(),
        path.display()
    );
}
